...
class Metric(StateManagementMixin, ABC):
    ...
    def _populate_from_scorer_response(self, scorer_response: Any) -> None:
        ...
        # Pre-compute optional common fields before the bulk set
        description = (
            ""
        ...
        # Common attributes for all metrics — use _sync_attrs to bypass dirty-tracking
        self._sync_attrs(
        ...
        if isinstance(self, LlmMetric):
            ...
            if not isinstance(scorer_response.scoreable_node_types, Unset) and scorer_response.scoreable_node_types:
                try:
                    node_level: StepType | None = StepType(scorer_response.scoreable_node_types[0])
        ...
        if isinstance(self, CodeMetric):
            ...
            if not isinstance(scorer_response.scoreable_node_types, Unset) and scorer_response.scoreable_node_types:
                try:
                    code_node_level: StepType | None = StepType(scorer_response.scoreable_node_types[0])
                except (ValueError, IndexError):
    ...
    def update(self, **kwargs: Any) -> Metric:
        ...
        The ``UpdateScorerRequest`` supports additional fields (``model_type``,
        ``ground_truth``, ``output_type``, etc.) but those are intentionally
        excluded here because they are managed at creation time or through
        dedicated methods.
        ...
        ------
            ...
            ValueError: If the metric has no ID set, is in DELETED state,
                is in FAILED_SYNC state (call ``refresh()`` first to re-sync),
                or unknown fields are passed.
            APIError: If the API returns a validation error or an empty response.
            Exception: If the underlying API call raises an infrastructure error.
...
class LlmMetric(Metric):
    ...
    def create(self) -> LlmMetric:
        ...
        try:
            ...
            # Update attributes from response — bypass dirty-tracking
            self._sync_attrs(
