            ...
            if not span_type or is_concludable_span_type(span_type):
                ...
                if logger.mode == "distributed" and not stack:
                    ...
                    if current_parent is not None:
                        ...
                        root_trace = logger.traces[-1] if logger.traces else None
                        is_stub_trace = root_trace is not None and root_trace.name == "stub_trace"

                        if not is_stub_trace and isinstance(current_parent, Trace):
                            current_parent.output = output
                            if redacted_output is not None:
                                current_parent.redacted_output = redacted_output
                            # Calculate trace duration from start to now (not just the workflow duration)
                            if current_parent.created_at:
                                current_parent.metrics.duration_ns = convert_time_delta_to_ns(
                                    _get_timestamp() - current_parent.created_at
                                )
                            if status_code is not None:
                                current_parent.status_code = status_code
                            # Update trace without marking complete - more workflows may be added
                            logger._update_trace_streaming(current_parent, is_complete=False)
            else:
                # Non-concludable spans (llm, tool, retriever) are just added to the parent
