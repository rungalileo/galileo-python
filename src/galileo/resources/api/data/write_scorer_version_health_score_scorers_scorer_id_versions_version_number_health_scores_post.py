...
def _get_kwargs(scorer_id: str, version_number: int, *, body: WriteHealthScoreRequest) -> dict[str, Any]:
    ...
    _kwargs: dict[str, Any] = {
        ...
        "path": f"/scorers/{scorer_id}/versions/{version_number}/health-scores",
    }
...
def _parse_response(
    *, client: ApiClient, response: httpx.Response
    ...
    if response.status_code == 200:
        return ScorerVersionHealthScoreEntry.from_dict(response.json())
...
def sync(
    scorer_id: str, version_number: int, *, client: ApiClient, body: WriteHealthScoreRequest
) -> HTTPValidationError | ScorerVersionHealthScoreEntry | None:
    """Write Scorer Version Health Score.
