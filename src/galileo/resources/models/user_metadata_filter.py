...
class UserMetadataFilter:
    ...
    Attributes:
        ...
        key (str):
        value (list[str] | str):
        name (Literal['user_metadata'] | Unset):  Default: 'user_metadata'.
    """

    operator: UserMetadataFilterOperator
    key: str
    value: list[str] | str
    name: Literal["user_metadata"] | Unset = "user_metadata"
    ...
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        ...
        name = cast(Literal["user_metadata"] | Unset, d.pop("name", UNSET))
        if name != "user_metadata" and not isinstance(name, Unset):
            raise ValueError(f"name must match const 'user_metadata', got '{name}'")

        user_metadata_filter = cls(operator=operator, key=key, value=value, name=name)
