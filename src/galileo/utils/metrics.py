...
def create_metric_configs(
    ...
    - GalileoMetricNames enum values (preferred — human-readable labels like "Context Adherence")
    - GalileoMetrics enum values (deprecated — internal names like "correctness")
    ...
    for metric in metrics:
        if isinstance(metric, GalileoMetricNames):
            label_searches.append((metric.value, None))
        elif isinstance(metric, GalileoMetrics):
            warnings.warn(
                "GalileoMetrics is deprecated. Use GalileoMetricNames instead.", DeprecationWarning, stacklevel=2
            )
            label_searches.append((metric.value, None))
