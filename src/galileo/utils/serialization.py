...
def _convert_langchain_content_block(block: dict) -> dict[str, Any]:
    """Convert a LangChain content block dict to Galileo IngestContentBlock shape.
    ...
    if kind == "text" or (kind is None and "text" in block):
        # LangChain "text" block or Responses API style {"text": "..."} without type
        text = block.get("text", "")
...
def _normalize_multimodal_content(content: Any) -> Any:
    ...
    if not isinstance(content[0], dict):
        return content
    return [_convert_langchain_content_block(item) for item in content]
