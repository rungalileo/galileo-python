...
from galileo.otel import _safe_instrument, _set_destination, enable_tracing
...
def mock_otel_components():
    with (
        patch("galileo.otel.OTLPSpanExporter") as mock_exporter,
...
def test_enable_tracing_with_params(
    ...
    mock_otel_components["provider"].return_value.add_span_processor.assert_called_once_with(
...
@patch("galileo.otel._safe_instrument")
def test_enable_tracing_with_env_vars(mock_safe_instrument, mock_set_destination, mock_otel_components, monkeypatch):
...
def test_enable_tracing_no_auto_instrument(
    ...
    enable_tracing(api_key="test_key", project_name="test_project", log_stream="test_logstream", auto_instrument=False)
...
def test_enable_tracing_no_session_id(
    ...
    enable_tracing(api_key="test_key", project_name="test_project", log_stream="test_logstream")
